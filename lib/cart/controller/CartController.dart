...
class CartController {
  ...
  Future<bool> addToCart(AddToCartData addToCartData) async {
    const endPointUrl =
        "https://swaraj.pythonanywhere.com/django/api/add_to_cart/";
  ...
  Future<CartData> getCartDetails(AddToCartData addToCartData) async {
    const endPointUrl =
        "https://swaraj.pythonanywhere.com/django/api/get_cart_details/";
    final parameters = addToCartData.getFormData(addToCartData);

    CartData serverMsg =
        await _httpRequestForGetCartData(endPointUrl, parameters);
    return serverMsg;
  }

  Future<bool> removeFromCart(DeleteFromCartData deleteFromCartData) async {
    const endPointUrl =
        "https://swaraj.pythonanywhere.com/django/api/remove_from_cart/";
    final parameters = deleteFromCartData.getFormData(deleteFromCartData);

    bool serverMsg =
        await _httpRequestForRemoveCartData(endPointUrl, parameters);
    return serverMsg;
  }
  ...
  Future<bool> _httpRequestForAddCartData(String url, FormData formData) async {
    ...
    try {
      ...
      if (response != null) {
        addToCartResponse =
            AddToCartResponse.getCartResponseFromHttpResponse(response);
  ...
  Future<CartData> _httpRequestForGetCartData(
  ...
  Future<bool> _httpRequestForRemoveCartData(
      String url, FormData formData) async {
    ...
    try {
      ...
      if (response != null) {
        deleteFromCartResponse =
            DeleteFromCartResponse.getRemoveCartResponseFromHttpResponse(
                response);
